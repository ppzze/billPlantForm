<template>
  <div class="first">
    <div class="mid">
      ...
      <div class="right">
        <div class="rtitle">出租车发票-识别结果</div>
...
export default {
  ...
  methods: {
    ...
    async sendImg() {
      ...
      await this.$http
        .post(
          `/rest/2.0/ocr/v1/taxi_receipt?access_token=${localStorage.access_token}`,
          formData
        )
        .then((res) => {
          console.log(res);
          ...
          if (typeof res.data.error_code !== "undefined") {
            ...
            this.show = true;
            if (localStorage.getItem("result3") == undefined) {
              let result = [];
              ...
              // let result = [];
              let be = JSON.parse(localStorage.getItem("result3"));
              console.log(be)
              be.push(res.data.words_result)
              // result.push(be)
             
              // result.push(res.data.words_result);
              localStorage.setItem("result3", JSON.stringify(be));
              console.log(localStorage.getItem("result3"));
              // console.log("1122");
            }
          }
