...
export default {
  ...
  methods: {
    ...
    exit(){
      ...
      let resexit = this.$http.post(
        `/proline/station/logOut/?stationId=${stationId}&staffId=${staffId}`
      );
      console.log(resexit)
    ...
    async getGongXu() {
      var posotionID = localStorage.positionId
      let resGongXu = await this.$http.get(
        `/proline/station/listErrorEquipment/?positionId=${posotionID}`
      );
    ...
    async getVideo(){
      ...
      let res  = await this.$http.get(
        `/proline/station/getFile/?positionId=${posotionID}`
      );
      ...
      for(var i= 0;i<res.data.data.length;i++){
        if(this.nextVideoIndex[0] == videoData[i].procedureId){
          ...
          this.playerOptions.sources[0].src = this.videoUrl
          console.log(videoData[i].fileUrl,this.videoUrl)
        }
      }
    },
    async fetchWork() {
      ...
      let res = await this.$http.get(
        `/proline/station/getProcess/?positionId=${posotionID}`
      );
