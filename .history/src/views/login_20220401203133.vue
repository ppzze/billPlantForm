...
export default {
  ...
  methods: {
    ...
    async fetch() {
      // console.log(localStorage.positionID)
    ...
    async fetchType() {
      ...
      console.log('我是login第二步请求登录码的真实数据',this.item,res.data.data.state);
      // {staffId: '', staffName: '', staffNum: '', state: 'WAIT_QR_CODE'}
      // 根据返回状态判断请求是否成功
      if (res.data.code == 20000) {
        this.state = res.data.data.state;
        if(this.state == 'WAIT_QR_CODE'){
          this.$router.push({path:'/login2',query: {item}})
          this.clear()
        }
        else if(this.state == 'WAIT_STAFF_CODE '){
          this.$router.push({path:'/login2',query: {this.item}});
          this.clear()
