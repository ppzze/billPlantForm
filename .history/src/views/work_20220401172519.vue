...
methods:{
  ...
  async getGongXu(){
    let resGongXu = await this.$http.get(`proline/station/listEquipmentState/?positionId=p1`);
    console.log('我是getGongXu里面的res',resGongXu)
    if(resGongXu.data.code == 20000){
      this.GONGXUtimer = setInterval(() => {this.getGongXu()}, 5000)
      clearInterval(this.GONGXUtimer);
      console.log('我是20000的时候',this.GONGXUtimer)
    }else{
      console.log('我是不是20000的时候')
     this.GONGXUtimer = setInterval(() => {this.getGongXu()}, 2000)
    }
  },
  async fetchWork() {
    console.log('我是工序中的状态码',this.resGongXu.data.code)
  ...
  async fetchVideo() {
      let res = await this.$http.get(`proline/station/getFile/?stationId=${this.stationId}`);
