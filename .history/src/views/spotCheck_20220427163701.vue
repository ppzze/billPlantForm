  ...
  data() {
    return {
      ...
      itemList: [
        {
          checkRuleId: "11",
          content: "请检测 USB设备 的工作情况 ",
          equipmentId: "2",
          type: "RESULT",
        },
        {
          checkRuleId: "12",
          content: "请检测斑马打印机 的工作情况 ",
          equipmentId: "1",
          type: "VALUE_SCOPE",
        },
      ],
  ...
  methods: {
    async checkEquipment() {
      let res = await this.$http.get(
        `proline/station/checkEquipment?positionId="p1"&userId='625EB404E4B05AD5E45AA75F1S1U'`
      );
    ...
    async listCheckData() {
      let res = await this.$http.get(
        `/proline/station/listCheckData?positionId="p1"`
      );
    ...
    getrecordList() {
      ...
      for (var i = 0; i < this.itemList.length; i++) {
        ...
        list.value = this.itemList[i].value;
        list.content = this.itemList[i].content;
        this.recordList.push(list);
      }
      setTimeout(this.commitCheckRecord(), 500);
    },
    commitCheckRecord() {
      let res = this.$http.post(`/proline/station/commitCheckRecord`, {
        checkStaffId: localStorage.checkStaffId,
