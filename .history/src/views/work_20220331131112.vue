...
methods:{
  ...
  async fetchVideo() {
      ...
      if (res.data.code == 20000) {
        ...
        {
          var video ={}
          // 下面这部分是 把 保存 fileUrl 和 procedureId = s1p1 拆成 s1（第几步） p1（第几集）
          video.fileUrl = res.data.data[i].fileUrl
          video.procedureIdstep = res.data.data[i].procedureId.split('p')[0]
          video.procedureIdprogress = res.data.data[i].procedureId.split('p')[1]
        }
